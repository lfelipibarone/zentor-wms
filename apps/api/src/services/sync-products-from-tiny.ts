...
async function setLastSyncAt(tenantId: string): Promise<void> {
  await prisma.systemSetting.upsert({
    where: { tenantId_key: { tenantId, key: TINY_PRODUCTS_LAST_SYNC_KEY } },
    create: {
      tenantId,
      key: TINY_PRODUCTS_LAST_SYNC_KEY,
...
export async function syncProductsFromTiny(params: {
  ...
  if (!tryAcquireTinySyncLock(params.tenantId, "products")) {
    return {
      ...result,
      tinyConnected: true,
      warning: "Sync de produtos já em andamento.",
    };
  }
  ...
  if (params.forceRestart) {
    await clearTinySyncCheckpoint(params.tenantId, TINY_PRODUCTS_CHECKPOINT_KEY);
  }
  ...
  try {
    while (offset < total) {
      ...
      for (const raw of page.items) {
        ...
        const listTipo = str(row.tipo)?.toUpperCase() ?? null;
        if (isTinyKitProductType(listTipo)) {
          await markExistingProductAsKit(params.tenantId, listSku);
          result.skipped += 1;
          continue;
        }
  ...
  return result;
    ...
    releaseTinySyncLock(params.tenantId, "products");
  }
}
